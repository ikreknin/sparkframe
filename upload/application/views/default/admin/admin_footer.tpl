		<div id="footer">
{highlighter}
			<div id="footer_content" class="container">
{footer_hook}

				<div id="copyright">
					<p>Copyright &copy; {copyright_years} <a href="http://tulks.com/">Igor Kreknin</a>. All Rights Reserved</p>
					<p>Powered by <a href="{site_url}">SparkFrame CMS</a> Version {current_version}</p>

					<br />
					<p><a href="http://validator.w3.org/check?uri=referer"><img src="http://www.w3.org/Icons/valid-xhtml10" alt="Valid XHTML 1.0 Strict" height="31" width="88" /></a>
					 <a href="http://jigsaw.w3.org/css-validator/check/referer"><img style="border:0;width:88px;height:31px" src="http://jigsaw.w3.org/css-validator/images/vcss" alt="Valid CSS!" /></a></p>
    
				</div>
			</div><!-- end footer content -->

		</div><!-- end footer -->
{before_closing_body_tag_hook}
{ckfinder_end}
	</body>
</html>