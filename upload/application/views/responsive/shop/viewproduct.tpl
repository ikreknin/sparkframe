...
{if '{visitor_user_id}'!='0'}
	<form method="post" action= "https://www.{sandbox_url}paypal.com/cgi-bin/webscr">
		...
		<input type="hidden" name="currency_code" value="{def_curr}">
...
{if '{pp_available}'=='y' && '{visitor_user_id}'!='0'}
		<i class="fa fa-credit-card fa-lg"></i> <input type="submit" value="{buy}">
{/if}

<br />

{if '{cart_available}'=='y' && '{visitor_user_id}'!='0'}
		<p><a href="{site_url}cart/add/{product_id}"><i class="fa fa-shopping-cart fa-lg"></i> {add_to_cart}</a></p>
{/if}


{if '{link_available}'=='1'}
<br />
		<p><a class="text" href="{site_url}{SHOP0}/get_link/{item_id}"><i class="fa fa-download"></i>
 {link_to_download}</a></p>
{/if}
...
<br />
{if '{visitor_user_id}'!='0'}
		<p><a class="text" href="{site_url}{SHOP0}/newopinion/{shop_id}"><i class="fa fa-comment fa-lg"></i> {new_opinion}</a></p>
{/if}
