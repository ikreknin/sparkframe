...
class Short_comments_list_extension
{
	private $registry;
	private $prefix;
	private $data = array('ext_name' => 'Short Comments List Extension', 'ext_description' => 'Short Comments List', 'ext_version' => '1.0', 'ext_order' => '1', 'ext_file_name' => 'short_comments_list_extension', 'ext_hook' => 'footer_3_hook');
	...
	public function index()
	{
		...
		$latest_comments_text = $this->registry->library('lang')->line('latest_comments_text');
		$result = '';
		$sql = 'SELECT *
    	FROM ' . $prefix . 'comments
    	LEFT JOIN ' . $prefix . 'articles ON com_article_id = article_id
		...
		if ($num != 0)
		{
			...
			{
				$result .= '<li>
				<a href="' . FWURL . $settings_site0 . 'user/id/' . $v["user_id"] . '">' . $v["author"] . '</a> : <a href="' . FWURL . $settings_site0 . '/more/' . $v["com_article_id"] . '/#' . $v["comment_id"] . '">' . $v["title"] . '</a>
				</li>';
			}
			$result .= '</ul>';
			$result .= '</div>';
