...
class Tagcontroller
{
	...
	public function __construct(Registry $registry, $directCall)
	{
		...
		if ($directCall == true)
		{
			...
			$this->registry->library('template')->page()->addTag('tag', $this->registry->library('lang')->line('tag'));
...
<script>
	...
	private function index()
	{
		...
		{
			$seg_1_space = urldecode(($this->seg_1));
			$table = $this->prefix . 'tags 
					LEFT JOIN ' . $this->prefix . 'tagged ON ' . $this->prefix . 'tagged.tag_id = ' . $this->prefix . 'tags.tag_id 
					LEFT JOIN ' . $this->prefix . 'articles ON ' . $this->prefix . 'tagged.article_id = ' . $this->prefix . 'articles.article_id';
			$condition = 'WHERE ' . $this->prefix . 'tags.tag_name = "' . $seg_1_space . '" 
					AND article_visible = 2
					AND articles_sys = "' . $this->sys_cms . '"
				ORDER BY ' . $this->prefix . 'articles.article_id ASC 
				LIMIT ' . $this->articles_per_page;;
			$pagination = $this->registry->library('paginate')->createLinksSimple($table, $this->articles_per_page, 3, 'tag/' . $this->seg_1 . '/page', $condition);
			$this->registry->library('template')->page()->addTag('pagination', $pagination);
			$this->registry->library('template')->page()->addTag('pagetitle', $this->registry->library('lang')->line('tag'));
			$this->registry->library('template')->page()->addTag('heading', $this->registry->library('lang')->line('tag'));
			$this->registry->library('template')->page()->addTag('current_tag', $seg_1_space);
			$sql = 'SELECT * 
				FROM ' . $this->prefix . 'tags 
					LEFT JOIN ' . $this->prefix . 'tagged ON ' . $this->prefix . 'tagged.tag_id = ' . $this->prefix . 'tags.tag_id 
					LEFT JOIN ' . $this->prefix . 'articles ON ' . $this->prefix . 'tagged.article_id = ' . $this->prefix . 'articles.article_id 
				WHERE ' . $this->prefix . 'tags.tag_name = "' . $seg_1_space . '" 
					AND article_visible = 2
					AND articles_sys = "' . $this->sys_cms . '"
				ORDER BY ' . $this->prefix . 'articles.article_id ASC 
				LIMIT ' . $this->articles_per_page;
			$cache = $this->registry->library('db')->cacheQuery($sql);
			$this->registry->library('template')->page()->addTag('articles', array('SQL', $cache));
			$this->registry->library('template')->build('header.tpl', 'tag.tpl', 'footer.tpl');
		}
		else
		{
		...
		if ($this->registry->library('authenticate')->isLoggedIn() == true || $this->registry->setting('settings_guests_allowed') == $this->guestsAllowed)
		{
			...
			$sql = 'SELECT * 
				...
				WHERE ' . $this->prefix . 'tags.tag_name = "' . $seg_1_space . '" 
					...
					AND articles_sys = "' . $this->sys_cms . '"
				ORDER BY ' . $this->prefix . 'articles.article_id ASC 
				LIMIT ' . $offset . ',' . $this->articles_per_page;
			$cache = $this->registry->library('db')->cacheQuery($sql);
			$this->registry->library('template')->page()->addTag('articles', array('SQL', $cache));
