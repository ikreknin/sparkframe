...
class U309{
	...
	{
//
		$text = 'SparkFrame Update from 3.0.8 to 3.0.9';
...
//
$sql2 = "UPDATE `{$prefix}cms` SET ver='309' WHERE cms_id='1'";
$this->registry->library('db')->execute($sql2);

$sql2 = "UPDATE `{$prefix}currency_list` SET currency_code='SPL' WHERE currency_id='129'";
$this->registry->library('db')->execute($sql2);

$sql2 = "ALTER TABLE `{$prefix}settings` ADD `def_curr` INT DEFAULT '0'";
$this->registry->library('db')->execute($sql2);
